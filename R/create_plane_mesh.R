create_plane_mesh <- function(corners, seg, create.uvs = FALSE){
	...
	# Create vertices
	vertices <- matrix(NA, nrow=n_vertices, ncol=3)
	...
	for(i in 0:(vertex_ct[2]-2)){
		...
		for(j in 1:(length(cross_idx)-1)){
			faces[n, ] <- c(cross_idx[j], cross_idx[j]+1, cross_idx[j+1])
			n <- n + 1

			faces[n, ] <- c(cross_idx[j]+1, cross_idx[j+1]+1, cross_idx[j+1])
			n <- n + 1
		}
	}
