svg.frame <- function(x = NULL, ranges = NULL, grid.lwd = 1, tick.lwd = 1, tick.num = 10, 
	...
	axis.label.opacity = 1, grid.opacity = 0.1, axis.col = rgb(0.5,0.5,0.5), 
	grid.col = rgb(0.8,0.8,0.8), text.col = 'black', z.index=0, lim.exact=FALSE, file=NULL){
	...
	svg_box <- svg.box(x=x, ranges=ranges, sides=1:3, grid.lwd=grid.lwd, tick.axes=c(2,3,2), 
		...
		text.col=text.col, z.index=z.index, lim.exact=lim.exact, file=file)
